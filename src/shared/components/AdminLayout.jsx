...
function SidebarContent({
  ...
  const menuFiltrado = MENU
    .map(grupo => ({
      ...grupo,
      items: grupo.items.filter(item => esAdmin() || tienePermiso(item.permiso))
    }))
    .filter(grupo => grupo.items.length > 0)

  const puedeDashboard = esAdmin() || tienePermiso('ver_dashboard')
  ...
  return (
    <div
      ...
      {puedeDashboard && (
        <div className="px-2 pt-2">
          <NavLink to="/admin" end
            className={({ isActive }) =>
              `flex items-center ${collapsed && !mobile
      ...
      <div className={`p-3 border-t border-gray-700
        ${collapsed && !mobile ? 'flex flex-col items-center gap-2' : 'space-y-1'}`}>
        {(!collapsed || mobile) && (
          <div className="relative">
            <button type="button"
              ...
              <div className="w-7 h-7 rounded-full bg-primary/30 flex items-center
            ...
            {perfilAbierto && (
              <div
                className="absolute bottom-full left-0 right-0 mb-1 rounded-xl
                  shadow-xl overflow-hidden animate-fadeIn z-10 border border-gray-700"
                style={{ backgroundColor: '#162210' }}>
                <button type="button"
                  onClick={() => { setPerfilAbierto(false); onCambiarContrasena() }}
                  className="w-full flex items-center gap-2.5 px-3 py-2.5 text-xs
                    text-gray-200 hover:bg-white/10 hover:text-primary transition-colors">
          ...
          <div className="flex flex-col gap-1">
            <button onClick={onCambiarContrasena}
              className="flex items-center justify-center p-1.5 rounded-lg
                text-gray-400 hover:text-primary hover:bg-white/10 transition-colors"
...
export default function AdminLayout() {
  ...
  return (
    <div className="flex h-screen bg-light-bg overflow-hidden">
      ...
      {modalContrasena && (
        <ModalCambiarContrasena onCerrar={() => setModalContrasena(false)} />
      )}
    </div>
  )
}