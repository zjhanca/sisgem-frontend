...
export default function Tabla({
  ...
  return (
    <div className="space-y-3 animate-fadeIn">
      ...
      {/* barra única: busqueda + registros + porPag + paginación */}
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {/* contador */}
        <span className="text-xs text-gray-400 dark:text-dark-text/35 shrink-0">
          {filtrados.length} registro{filtrados.length !== 1 ? 's' : ''}
        </span>

        {/* por página */}
        <select value={porPag} onChange={e => handlePorPag(e.target.value)}
          className="campo-input w-[90px] text-xs py-1 h-7 shrink-0">
        ...
        {totalPaginas > 1 && (
          <div className="flex items-center gap-1 ml-auto">
            <span className="text-xs text-gray-400 dark:text-dark-text/35 mr-1 shrink-0">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-light-bg dark:bg-dark-bg border-b border-gray-100 dark:border-dark-border/60">
              ...
              {acciones && <th className="tabla-header text-right">Acciones</th>}
            </tr>
          </thead>
