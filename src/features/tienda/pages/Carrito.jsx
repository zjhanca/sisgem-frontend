...
export default function Carrito({ carrito, setCarrito }) {
  const {
    usuario, tipoVenta, setTipoVenta, notas, setNotas,
    enviando, total, cambiarCantidad, quitar, handlePedido,
  ...
  if (carrito.length === 0) {
    return (
      <div className="min-h-screen bg-light-bg dark:bg-dark-bg flex flex-col items-center justify-center px-4">
        ...
        <h2 className="text-xl font-semibold text-light-text dark:text-dark-text mb-2">Tu Carrito está Vacío</h2>
    ...
    <div className="min-h-screen bg-light-bg dark:bg-dark-bg">
      <div className="max-w-2xl mx-auto px-4 py-6">
            ...
            {[
              ...
              <button key={op.val} type="button" onClick={() => setTipoVenta(op.val)}
                className={`flex items-center gap-3 p-3 rounded-xl border-2 text-left transition-all ${
                  tipoVenta === op.val
                    ? op.val === 'domicilio' ? 'border-blue-500 bg-blue-500/10' : 'border-primary bg-primary/10'
                    : 'border-gray-200 dark:border-dark-border'
                }`}>
                <op.icon size={18} className={tipoVenta === op.val ? (op.val === 'domicilio' ? 'text-blue-500' : 'text-primary') : 'text-gray-400'} />
                <div>
                  <p className={`text-sm font-medium ${tipoVenta === op.val ? (op.val === 'domicilio' ? 'text-blue-500' : 'text-primary') : 'text-light-text dark:text-dark-text'}`}>{op.label}</p>
        ...
        <div className="space-y-2 mb-4">
          {carrito.map(p => (
            <div key={p.id} className="flex items-center gap-3 p-3 bg-light-card dark:bg-dark-card rounded-xl border border-gray-100 dark:border-dark-border">
              {p.imagen_url
                ? <img src={p.imagen_url} alt={p.nombre} className="w-14 h-14 object-cover rounded-lg shrink-0" onError={e => e.target.style.display='none'} />
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div className="flex items-center gap-1">
                  <button onClick={() => cambiarCantidad(p.id, -1)} className="w-7 h-7 rounded-lg border border-gray-200 dark:border-dark-border flex items-center justify-center hover:border-primary/40 transition-colors"><Minus size={11} className="text-gray-500" /></button>
                  <span className="w-8 text-center text-sm font-medium text-light-text dark:text-dark-text">{p.cantidad}</span>
                  <button onClick={() => cambiarCantidad(p.id, 1)} className="w-7 h-7 rounded-lg border border-gray-200 dark:border-dark-border flex items-center justify-center hover:border-primary/40 transition-colors"><Plus size={11} className="text-gray-500" /></button>
                </div>
                <span className="text-sm font-semibold text-primary w-16 text-right">{formatPrecio(p.precio * p.cantidad)}</span>
                <button onClick={() => quitar(p.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-400/10 transition-colors"><Trash2 size={14} className="text-red-400" /></button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mb-4">
          ...
          <textarea value={notas} onChange={e => setNotas(e.target.value)}
            rows={2} className="campo-input resize-none" placeholder="Instrucciones especiales para tu pedido..." />
        </div>
        ...
        <div className="bg-light-card dark:bg-dark-card rounded-xl border border-gray-100 dark:border-dark-border p-4 mb-4">
          ...
          {tipoVenta === 'domicilio' && (
            <div className="flex justify-between text-sm text-gray-500 mb-1">
              <span>Domicilio</span><span className="text-xs text-gray-400">Se calcula al confirmar</span>
            </div>
          )}
        ...
        {!usuario ? (
          <div className="space-y-2">
            <p className="text-xs text-center text-gray-400">Debes iniciar sesión para confirmar tu pedido</p>
            <Link to="/login" className="btn-primary w-full justify-center py-3">Iniciar Sesión</Link>
