...
export function useDashboard() {
  ...
  const { data: ventasMes = [] } = useQuery({
    queryKey: ['ventas-mes'],
    queryFn:  dashboardService.getVentasMes,
  })

  const { data: bajoStock = [] } = useQuery({
    queryKey: ['bajo-stock'],
    queryFn:  dashboardService.getBajoStock,
    refetchInterval: 60000,
  })

  const data = rawData ? { ...rawData, bajo_stock: bajoStock } : null
  ...
  return {
    data, isLoading,
    ventasMes, bajoStock,
    ventasGrafica, totalSemana, totalMes,
