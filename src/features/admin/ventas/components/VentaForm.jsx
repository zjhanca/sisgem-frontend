...
import { Search, Scan, Trash2, CreditCard, Clock, AlertTriangle, UserPlus } from 'lucide-react'
...
export default function VentaForm({
  ...
  // Hook de clientes para el formulario embebido
  const {
  ...
  const abrirNuevoCliente = () => {
    abrirModalCliente()     // inicializa el form limpio
  ...
  useEffect(() => {
    const handler = e => {
      if (dropdownRef.current && !dropdownRef.current.contains(e.target)) {
        setClienteBusqueda(''); setClienteDropdown(false)
      }
    }
  ...
  return (
    <>
                  ...
                  {clienteSeleccionado && sinCupo && form.tipo_pago === 'fiado' && (
                    <div className="flex items-start gap-2 p-3 rounded-lg bg-red-50 border border-red-200">
                      ...
                      <p className="text-xs text-red-600">
                        Este cliente no tiene cupo de fiado disponible. Elige "Pago Total".
                      </p>
                    </div>
                  )}
      ...
      {/* Modal nuevo cliente embebido — se abre encima del modal de venta */}
      <ClienteForm
