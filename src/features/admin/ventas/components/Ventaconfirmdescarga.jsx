...
export default function VentaConfirmDescarga({ confirmDescarga, setConfirmDescarga }) {
  ...
  const confirmar = async () => {
    ...
    try {
      if (confirmDescarga.tipo === 'comprobante') {
        if (formato === 'tirilla') {
          await descargarPDF(
            `/reportes/pedido/${confirmDescarga.id}/tirilla`,
            `comprobante-${confirmDescarga.id}.pdf`
          )
        } else {
          await descargarPDF(
            `/reportes/pedido/${confirmDescarga.id}`,
            `comprobante-${confirmDescarga.id}.pdf`
          )
        }
      } else {
  ...
  const etiqueta = confirmDescarga?.tipo === 'comprobante'
    ? `comprobante de la venta ${confirmDescarga.id}`
    : 'reporte de ventas'
    ...
    <Modal abierto={!!confirmDescarga} onCerrar={cerrar} bloquearCierre
      ...
      {confirmDescarga && (
        <div className="space-y-4">
          ...
          {/* Selector de formato solo para comprobante */}
          {confirmDescarga.tipo === 'comprobante' && (
            <div>
