...
export default function ProductoConfirmDescarga({ abierto, setAbierto }) {
  ...
  const confirmar = async () => {
    setDescargando(true)
    try {
      await descargarPDF('/reportes/productos', 'reporte-productos.pdf')
    } finally {
    ...
    <Modal abierto={abierto} onCerrar={cerrar} bloquearCierre titulo="Descargar Reporte" ancho="max-w-sm">
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          ¿Deseas descargar el <span className="font-semibold text-light-text">reporte de productos</span> en formato PDF?
        </p>
