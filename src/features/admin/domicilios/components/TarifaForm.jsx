...
export default function TarifaForm({ tarifas, formTarifa, setFormTarifa, handleGuardarTarifa, guardandoTarifa, modalEliminarTarifa, setModalEliminarTarifa, eliminarTarifa, eliminandoTarifa }) {
  const columnas = [
      ...
      <div className="p-4 rounded-xl border border-gray-200 dark:border-dark-border bg-light-card dark:bg-dark-card">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            ...
            <input value={formTarifa.barrio} onChange={e => setFormTarifa(p => ({ ...p, barrio: e.target.value }))}
              className="campo-input text-xs" placeholder="Ej: Laureles" />
          </div>
          <div>
            <label className="campo-label">Zona</label>
            <input value={formTarifa.zona} onChange={e => setFormTarifa(p => ({ ...p, zona: e.target.value }))}
              className="campo-input text-xs" placeholder="Ej: Norte" />
          </div>
          <div>
            <label className="campo-label">Tarifa *</label>
            <input type="number" step="0.01" value={formTarifa.tarifa} onChange={e => setFormTarifa(p => ({ ...p, tarifa: e.target.value }))}
              className="campo-input text-xs" placeholder="0.00" />
          </div>
          <div>
            <label className="campo-label">Distancia (km)</label>
            <input type="number" step="0.1" value={formTarifa.distancia_km} onChange={e => setFormTarifa(p => ({ ...p, distancia_km: e.target.value }))}
              className="campo-input text-xs" placeholder="0.0" />
          </div>
        </div>
        <div className="flex justify-end mt-3">
          <button onClick={handleGuardarTarifa} disabled={guardandoTarifa} className="btn-primary">
      ...
      <Modal abierto={modalEliminarTarifa.abierto} onCerrar={() => setModalEliminarTarifa({ abierto: false, item: null })}
        ...
        <div className="space-y-4">
          <p className="text-sm">¿Eliminar la tarifa del barrio
            <span className="font-medium text-primary"> {modalEliminarTarifa.item?.barrio}</span>?
          </p>
          <div className="flex justify-end gap-2 pt-2 border-t border-gray-200 dark:border-dark-border">
            ...
            <button onClick={() => eliminarTarifa.mutate(modalEliminarTarifa.item.id)} disabled={eliminandoTarifa}
