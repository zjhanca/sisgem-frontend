...
export default function OrdenDetalle({
  ...
  return (
    <>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              {orden.notas && (
                <div className="col-span-2 min-w-0">
                  <p className="campo-label">Notas</p>
                  <p onClick={() => setNotaAbierta(true)} title="Ver nota completa"
                    className="text-gray-500 italic truncate cursor-pointer hover:text-primary hover:underline">
                    {orden.notas}
                  </p>
                </div>
              )}
      ...
      <Modal abierto={notaAbierta} onCerrar={() => setNotaAbierta(false)} titulo="Nota completa" ancho="max-w-sm">
        <p className="text-sm text-gray-600 whitespace-pre-wrap break-words">{orden?.notas}</p>
      </Modal>
    </>
  )
}