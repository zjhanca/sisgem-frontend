...
export default function CategoriaDetalle({ modalDetalle, setModalDetalle, abrirModal, toggleEstado }) {
  ...
  return (
    <Modal abierto={modalDetalle.abierto} onCerrar={cerrar} bloquearCierre titulo="Detalle de Categoría">
      {item && (
        <div className="space-y-4">
          <div className="flex items-center gap-4 p-3 rounded-xl bg-gray-50">
            <div className="w-14 h-14 rounded-full bg-primary flex items-center justify-center shrink-0 overflow-hidden">
              {item.icono
                ? <img src={item.icono} alt={item.nombre} className="w-9 h-9 object-contain"
                    onError={e => e.target.style.display='none'} />
                : <span className="text-xl font-bold text-white">{item.nombre?.charAt(0).toUpperCase()}</span>
              }
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-light-text truncate">{item.nombre}</p>
