        ...
        <div>
          ...
          <div className="flex gap-3 items-start">
            <div className="shrink-0 w-16 h-16 rounded-full bg-primary flex items-center justify-center overflow-hidden border-2 border-primary/30">
              {form.icono
                ? <img src={form.icono} alt="preview" className="w-10 h-10 object-contain"
            ...
            <div className="flex-1 space-y-2">
              <button type="button" onClick={() => fileRef.current?.click()}
                className="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed
                  border-gray-200 text-xs text-gray-400 hover:border-primary/40 hover:text-primary transition-colors">
              ...
              <div className="relative">
                ...
                <input value={form.icono?.startsWith('data:') ? '' : (form.icono ?? '')}
                  onChange={e => handleChange('icono', e.target.value)}
                  className="campo-input pl-7 text-xs" placeholder="O pegar URL de imagen..." />
              </div>
              {form.icono && (
                <button type="button" onClick={() => handleChange('icono', '')}
