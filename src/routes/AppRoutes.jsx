...
function RutaAdmin({children}){const{usuario,cargando}=useAuth();if(cargando)return null;if(!usuario)return <Navigate to='/login' replace />;if(+usuario.rol_id!==1)return <Navigate to='/perfil' replace />;return children}
function RutaCliente({children}){const{usuario,cargando}=useAuth();if(cargando)return null;if(!usuario)return <Navigate to='/login' replace />;return children}
