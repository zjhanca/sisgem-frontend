...
export default function Marcas() {
  ...
  const eliminar = useMutation({
    ...
    onError: err => toast.error('no se puede eliminar, tiene productos asociados')
  })
 
  const abrirModal = (item = null) => {
    setForm(item ? {
      nombre: item.nombre, descripcion: item.descripcion || '',
      logo: item.logo || '', proveedor_id: item.proveedor_id || '',
  ...
  const columnas = [
    ...
    { key: 'total_productos', label: 'Productos',
      render: r => <span className="badge-proceso">{r.total_productos} productos</span>
    },
  ...
  return (
    <div>
          ...
          <div>
            ...
            {form.logo && (
              <img src={form.logo} alt="preview"
                className="mt-2 w-10 h-10 object-contain rounded border border-gray-200 dark:border-dark-border"
            ...
            <div className="flex items-start gap-4">
              {modalDetalle.item.logo ? (
                <img src={modalDetalle.item.logo} alt={modalDetalle.item.nombre}
                  className="w-16 h-16 object-contain rounded-xl border border-gray-200 dark:border-dark-border"
                  onError={e => e.target.style.display = 'none'} />
              ) : (
              ...
              <div className="flex-1">
                <h3 className="font-semibold">{modalDetalle.item.nombre}</h3>
                ...
                {modalDetalle.item.descripcion && (
                  <p className="text-xs text-gray-400 mt-1">{modalDetalle.item.descripcion}</p>
                )}
      ...
      <Modal abierto={modalEliminar.abierto}
        ...
        <div className="space-y-4">
          <p className="text-sm">estas seguro que deseas eliminar la marca
