...
export default function Pagos() {
  ...
  const totalPagadoPorPedido = pedidos.reduce((acc, p) => {
    const pagosDelPedido = pagos.filter(pg => pg.pedido_id === p.id && pg.estado_id !== 8)
    acc[p.id] = pagosDelPedido.reduce((s, pg) => s + +pg.monto, 0)
    return acc
  }, {})
  ...
  const totalPagado = totalPagadoPorPedido[+form.pedido_id] || 0
  const montoPendiente = Math.max(0, totalPedido - totalPagado)
  const pagoCompleto = montoPendiente === 0
 
  const validar = () => {
    const e = {}
    if (!form.pedido_id) e.pedido_id = 'selecciona un pedido'
    if (!form.monto || +form.monto <= 0) e.monto = 'monto invalido'
    if (pagoCompleto) e.monto = 'el pedido ya esta completamente pagado'
    return e
  }
  ...
  const pagosFiltrados = pagos.filter(p => !filtroEstado || p.estado_id === +filtroEstado)
  ...
  const columnas = [
    ...
    { key: 'metodo',    label: 'Metodo', render: r => r.metodo || '-' },
    { key: 'estado',    label: 'Estado',
      render: r => (
        <span className={r.estado?.toLowerCase().includes('anula') ? 'badge-anulado' : 'badge-activo'}>
          {r.estado || 'activo'}
        </span>
      )
    },
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <select value={filtroEstado} onChange={e => setFiltroEstado(e.target.value)} className="campo-input w-36 text-xs">
          ...
          <option value="8">anulados</option>
        </select>
      ...
      <Tabla columnas={columnas} datos={pagosFiltrados} sinBusqueda
        acciones={fila => (<>
          ...
          {!fila.estado?.toLowerCase().includes('anula') && (
            <button onClick={() => setModalAnular({ abierto: true, pago: fila })}
              className="btn-ghost hover:text-red-400" title="anular"><Ban size={14} /></button>
          )}
        </>)}
      />
 
      {/* MODAL NUEVO PAGO */}
      <Modal abierto={modalNuevo} onCerrar={() => setModalNuevo(false)} titulo="registrar pago">
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {form.pedido_id && (
            <div className="p-3 rounded-lg bg-light-bg dark:bg-dark-bg text-xs space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-green-400">{formatPrecio(totalPagado)}</span>
              </div>
              <div className="flex justify-between font-semibold border-t border-gray-200 dark:border-dark-border pt-1">
                ...
                <span className={pagoCompleto ? 'text-green-400' : 'text-primary'}>
                  {pagoCompleto ? '✓ pagado completo' : formatPrecio(montoPendiente)}
                </span>
              </div>
            </div>
          )}
          ...
          <div>
            <label className="campo-label">metodo de pago</label>
        ...
        {modalDetalle.pago && (
          <div className="space-y-3 text-sm">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><p className="campo-label">monto</p>
                <p className="text-primary font-bold text-base">{formatPrecio(modalDetalle.pago.monto)}</p></div>
              <div><p className="campo-label">metodo</p><p>{modalDetalle.pago.metodo}</p></div>
              <div><p className="campo-label">estado</p>
                <span className={modalDetalle.pago.estado?.toLowerCase().includes('anula') ? 'badge-anulado' : 'badge-activo'}>
                  {modalDetalle.pago.estado || 'activo'}</span></div>
              <div><p className="campo-label">fecha</p><p>{formatFecha(modalDetalle.pago.created_at)}</p></div>
            </div>
            {!modalDetalle.pago.estado?.toLowerCase().includes('anula') && (
      ...
      <Modal abierto={modalAnular.abierto} onCerrar={() => setModalAnular({ abierto: false, pago: null })}
        ...
        <div className="space-y-4">
          <p className="text-sm">estas seguro de anular el pago
